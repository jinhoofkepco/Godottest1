...
`BattleSimulation`은 30 Hz 고정 틱에서 논리 `(col,row)` 좌표만 사용합니다. 유닛은 `PackedInt32Array`, `PackedVector2Array`, `PackedFloat32Array` 등 구조별 배열에 저장되며 유닛당 Node·시그널·`_physics_process`가 없습니다. 전진·적 추적·아군 분리력의 가중합으로 이동하고, 2.5셀 탐지 범위를 11×22 공간 버킷으로 제한해 전체 쌍 비교를 피합니다. 스폰마다 속도 ±10%와 x 위치 ±0.3셀 차이를 주며, 공격 중에는 정지하고 타깃 방향으로 짧게 런지합니다.
...
2026-07-19 Apple M4, Godot 4.7 stable 로컬 측정에서 2.5셀 탐지와 분리력을 사용하는 데이터 유닛 400개·30틱 워밍업 뒤 300틱은 평균 **8.149 ms**, p95 **10.469 ms**, 최대 **19.030 ms**, 최대 버킷 후보 검사 **86,286회**였습니다. 매 틱 실제 최근접 유닛·건물을 다시 비교하는 조건에서 평균과 p95는 60 FPS 프레임 예산 16.667 ms보다 낮으며, 테스트는 평균 또는 p95가 이 예산을 넘으면 실패합니다. Android/CI의 절대 수치는 기기에 따라 달라집니다.
