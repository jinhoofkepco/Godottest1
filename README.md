...
Godot 4.5 stable .NET + C#/GDScript로 만든 540×960 세로형 아이소메트릭 군단 전선 시뮬레이션입니다. 시뮬레이션 코어는 C#, HUD·입력·FX는 GDScript이며 보병·공성병기·드래곤·건물은 CC0 3D 모델을 자체 베이크한 스프라이트입니다. Android 릴리스 메타데이터는 **1.3.0 (version code 12)** 입니다.
...
- 전진 집결지는 대기 병력 10명이 모이면 현재 병과 구성 그대로 군단을 편성해 상대 HQ로 출격합니다.
...
Godot 4.5 자동 밸런스 경로는 파랑이 아무것도 짓지 않을 때 **172.4초 DEFEAT**, 양측이 경제·카운터·집결 명령을 모두 수행하는 고정 시드 경기에서 **351.6초 VICTORY**, 점유율 77.8%를 기록했습니다. 대응 경기 목표 5–7분을 충족하며 HQ HP는 양측 2400입니다.

이전 AI의 실패 원인은 `CountBarracks >= 3`에서 영구 반환해 첫 3개 건물 뒤 경제·카운터 판단을 모두 멈춘 것이었습니다. 개편 직후에는 집결지가 가장자리 열에 지어지고 1.7셀 반경에 대기열 선두 3–4명만 들어가 출격 10명을 영원히 못 채우는 두 번째 결함도 계측으로 확인했습니다. 현재는 중앙 우선 배치와 2.8셀 집결 반경을 사용하며, 120초 진단에서 7개 군단/70명이 출격하고 점유율을 46.2%까지 실제로 이동시켰습니다.

동골드 카운터 매트릭스 20회 결과는 RANGED→MELEE **20/20**, MELEE→SIEGE **20/20**, DRAGON→RANGED **19/20**, DRAGON→SIEGE **20/20**입니다. AI 건전성 50+50 결과는 무행동 플레이어 **50/50 패배, 평균 174.5초**, AI 대 AI **파랑 27승·타임업 무승부 26회, 무승부 보정 점수율 50%, 평균 401.3초**, 골드 적체/건설 정지 **0건**입니다.
...
렌더 p95는 **84.7% 감소**했고 로컬 목표 ≤2 ms를 달성했습니다. C# 단독 30셀 델타 마샬링은 평균 0.015 ms/p95 0.017 ms였습니다. GitHub 공유 러너의 software-renderer 편차를 고려해 CI 렌더 게이트는 25 ms로 완화하지만, 구조 테스트가 정확히 30개 instance만 갱신되고 전체 sync·transform 재기록·영토 FX 객체가 없음을 별도로 강제합니다.
...
이 버전의 GitHub Actions 검증 APK를 바로 받을 수 있습니다: [godottest1 v1.3.0 APK 직접 다운로드](https://github.com/jinhoofkepco/Godottest1/releases/download/v1.3.0/godottest1.apk)
