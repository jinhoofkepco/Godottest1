...
Godot 4.5 stable + GDScript로 만든 540×960 세로형 아이소메트릭 양군 전선 시뮬레이션입니다. 타일·건물·HUD·FX와 비행 드래곤은 코드 도형이고, 보병은 CC0 KayKit 3D 모델을 자체 베이크한 스프라이트입니다. Android 릴리스 메타데이터는 **0.6.0 (version code 5)** 입니다.
...
렌더러는 전역 Y 정렬 보병 MultiMesh 하나와 팀별 드래곤 MultiMesh 두 개로 군단을 일괄 처리합니다. 보병 배치는 동일 QuadMesh와 팀별 아틀라스 두 장으로 만든 2-layer Texture2DArray를 공유하며, per-instance custom data를 CanvasItem 셰이더 vertex→fragment varying으로 전달해 팀·병과·8방향·idle/walk/attack/death 프레임, HP 밝기와 사망 페이드를 선택합니다. 모든 보병을 한 배열에서 화면 Y로 정렬하므로 서로 다른 팀·병과가 겹쳐도 아이소 깊이가 유지됩니다. 방향은 packed velocity를 45° 단위로 양자화하고 공격 프레임은 쿨다운·런지와 동기화됩니다. 사망 유닛은 룰에서는 즉시 제거되지만 렌더러가 0.48초 death 프레임만 보존합니다. 모든 병력의 타원 그림자는 MultiMesh 하나가 추가로 처리하며 유닛별 Node는 없습니다. 넓은 날개의 드래곤은 기존 절차 실루엣을 유지합니다.

## CC0 모델과 재현 가능한 베이크

- 모델: KayKit - Character Pack: Adventurers 1.0의 `Knight.glb`(근접), `Rogue_Hooded.glb`(원거리)
- 공식 출처: https://github.com/KayKit-Game-Assets/KayKit-Character-Pack-Adventures-1.0
- 라이선스: `assets/source/kaykit/LICENSE.txt`의 CC0 1.0 Universal
- 원본 GLB와 upstream revision은 `assets/source/kaykit/README.md`에 보존했습니다.

`tools/sprite_baker/bake_sprites.gd`는 모델 경로, 애니메이션명, 방향 수, 아이소 카메라 pitch, 셀 크기, 출력 경로를 인자로 받는 범용 Godot CLI입니다. 투명 SubViewport에서 팀 컬러를 머티리얼에 직접 적용해 8방향 × `idle 2 / walk 6 / attack 4 / death 4`를 캡처합니다. 결과는 팀별 1536×1536 RGBA 시트 두 장(각 셀 96×96, 각 파일 약 1.5 MB, 비압축 합계 18 MiB)과 메타 JSON이며 2048² 제한 안에 있습니다. 재실행법은 `tools/sprite_baker/README.md`에 있습니다.
...
2026-07-20 Apple M4, 배포 기준과 같은 Godot 4.5 stable에서 22개 열 전체에 근접 200기·원거리 200기를 균등 배치하고 팀별 혼잡 플로우 재계산과 HQ 자동 방어까지 포함해 30틱 워밍업 뒤 300틱을 측정했습니다. 평균 **14.428 ms**, p95 **25.566 ms**, 최대 **34.075 ms**, 틱당 최대 적 후보 검사 **837회**였습니다. 로컬 게이트는 평균 16.667 ms 미만, 주기 재계산을 포함한 p95 30 ms 미만, 후보 검사 12,000회 미만입니다. p95는 30 Hz 시뮬레이션의 33.333 ms 틱 기한보다 낮으며, 최대 단일 틱도 숨기지 않고 함께 기록합니다. GitHub 호스티드 러너는 공유 VM 편차를 고려해 평균 30 ms, 주기적 플로우 재계산 p95 55 ms 게이트를 사용하며 로컬 성능 기준을 대체하지 않습니다.
...
마지막 명령은 전체 오프닝(방어탑·드래곤 레어 포함), 파랑 우세, 파랑 열세, 전선 혼전, 영속 측면 점유, 두 병목 분산, 8방향 보병 클로즈업의 540×960 PNG 일곱 장을 만듭니다. 클로즈업은 `build/smoke_infantry_closeup.png`이며 방향·두 병과·공격 런지·사망 프레임·블롭 그림자를 한 화면에서 확인합니다.
