...
- 스포너가 만든 근접 유닛은 2.5셀 안의 적을 2D로 추적하고, 가까운 아군과 서로 밀어내며 자동 진군·교전합니다. 90% 점유, HQ 파괴, 또는 3분 타임업 우세 판정으로 승패가 결정됩니다.
- 스포너를 짓지 않으면 약 123초에 밀려 패배하고, 시작 골드로 세 곳에 스포너를 지으면 약 129초에 적 HQ를 돌파해 승리하는 두 경로를 자동 테스트로 검증합니다.

## 로직과 렌더링 경계

`BattleSimulation`은 30 Hz 고정 틱에서 논리 `(col,row)` 좌표만 사용합니다. 유닛은 `PackedInt32Array`, `PackedVector2Array`, `PackedFloat32Array` 등 구조별 배열에 저장되며 유닛당 Node·시그널·`_physics_process`가 없습니다. 전진·적 추적·아군 분리력의 가중합으로 이동하고, 2.5셀 탐지 범위를 팀별 11×22 공간 버킷으로 제한해 전체 쌍 비교를 피합니다. 최근 타깃 거리보다 가까질 수 있는 버킷만 재검사하되 유닛·건물 전체에서 실제 최근접을 유지합니다. 스폰마다 속도 ±10%와 x 위치 ±0.3셀 차이를 주며, 공격 중에는 정지하고 타깃 방향으로 짧게 런지합니다.
...
2. **유닛 사망:** 유닛 크기에 맞춘 작은 팀 색 팝과 사각 파편으로 일반 타격과 구분됩니다.
...
6. **HQ 피격:** 큰 동심원·사선 붉은 플래시가 주 피드백이고, 흔들림은 0.5초 재점화 제한과 `trauma²` 감쇠를 적용한 부드러운 진동으로 평상시 3px 이하입니다. HQ 파괴만 0.4초 동안 최대 6px를 허용합니다.

전투 중인 유닛은 타깃 방향 짧은 런지와 흰 타격 스파크로 구분됩니다. 옆 열의 적도 탐지해 모이고, 아군 분리력이 완전한 겹침을 풀어 전선에 판별 가능한 혼전 클러스터가 생깁니다.
...
2026-07-19 Apple M4, Godot 4.7 stable 로컬 측정에서 2.5셀 탐지와 분리력을 사용하는 데이터 유닛 400개·30틱 워밍업 뒤 300틱은 평균 **5.139 ms**, p95 **6.396 ms**, 최대 **11.249 ms**, 최대 버킷 후보 검사 **16,440회**였습니다. 매 틱 실제 최근접 유닛·건물을 보장하는 조건에서 평균과 p95는 60 FPS 프레임 예산 16.667 ms보다 낮으며, 테스트는 후보 검사 30,000회 또는 평균/p95 예산을 넘으면 실패합니다. Android/CI의 절대 수치는 기기에 따라 달라집니다.

렌더 가능한 로컬 환경에서는 다음 명령이 초반/파랑 우세/파랑 열세/전선 혼전 클러스터 네 장을 각각 540×960 PNG로 만듭니다.

```bash
godot --display-driver macos --rendering-method gl_compatibility --audio-driver Dummy --path . -s tests/smoke_capture.gd
```

Linux CI는 같은 스크립트를 Xvfb에서 실행하며 `build/smoke_opening.png`, `build/smoke_advantage.png`, `build/smoke_disadvantage.png`, `build/smoke_cluster.png`를 확인합니다.
