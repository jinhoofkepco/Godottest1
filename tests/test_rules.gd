...
func _test_rally_defense_overflow_and_destroy_fallback() -> void:
	...
	var legion_states := PackedInt32Array(defended.legion_states)
	var gathering_index := legion_states.find(LEGION_GATHERING)
	var marching_index := legion_states.find(LEGION_MARCHING)
	_expect(gathering_index >= 0, "DEFEND rally holds a garrison in formation")
	_expect(marching_index >= 0, "DEFEND overflow launches automatically")
	...
	if marching_index >= 0:
		var marching_id := int(defended.legion_ids[marching_index])
		var member_index := PackedInt32Array(defended.unit_legion_ids).find(marching_id)
		_expect(int(defended.legion_member_counts[marching_index]) == 1 and PackedInt32Array(defended.unit_legion_ids).count(marching_id) == 1, "single DEFEND overflow legion owns exactly one real member")
		_expect(member_index >= 0 and int(simulation.call("GetRenderSnapshot").legion_banner_count) == 2, "each DEFEND banner corresponds to a non-empty legion")
		if member_index >= 0:
			var member_id := int(defended.unit_ids[member_index])
			var start_member := Vector2(defended.unit_positions[member_index])
			var start_anchor := Vector2(defended.legion_anchors[marching_index])
			for tick in range(180): simulation.call("Step", 1.0 / 30.0)
			var advanced: Dictionary = simulation.call("GetDebugSnapshot")
			var advanced_legion_index := PackedInt32Array(advanced.legion_ids).find(marching_id)
			var advanced_member_index := PackedInt32Array(advanced.unit_ids).find(member_id)
			_expect(advanced_legion_index >= 0 and Vector2(advanced.legion_anchors[advanced_legion_index]).y < start_anchor.y - 2.0, "DEFEND overflow legion anchor advances toward the enemy")
			_expect(advanced_member_index >= 0 and Vector2(advanced.unit_positions[advanced_member_index]).y < start_member.y - 0.75, "DEFEND overflow member exits through the stationary garrison")
			if advanced_legion_index >= 0 and advanced_member_index >= 0:
				var advanced_member := Vector2(advanced.unit_positions[advanced_member_index])
				_expect(advanced_member.distance_to(rally_anchor) > GameConfig.RALLY_ARRIVAL_RADIUS, "DEFEND overflow member leaves the rally arrival radius")
				_expect(Vector2(advanced.legion_anchors[advanced_legion_index]).distance_to(advanced_member) <= 2.0, "DEFEND overflow banner stays coupled to its real member")
