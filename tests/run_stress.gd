...
func _run() -> void:
	...
	print("STRESS PASS: initial=%d remaining=%d ticks=%d avg_ms=%.3f p95_ms=%.3f max_candidates=%d" % [
		UNIT_COUNT,
	...
	if average_ms >= 33.333 or p95_ms >= 33.333:
		push_error("STRESS FAILED: fixed simulation tick exceeded 30 Hz budget")
		quit(1)
		return
	quit(0)
