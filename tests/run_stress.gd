...
func _run() -> void:
	...
	print("STRESS PASS: initial=%d mixed_kinds=true columns=%d remaining=%d warmup=%d ticks=%d avg_ms=%.3f p95_ms=%.3f max_ms=%.3f max_candidates=%d budget_ms=%.3f" % [
		UNIT_COUNT,
	...
	if maximum_candidate_checks >= MAX_CANDIDATE_CHECKS:
		...
		quit(1)
		return
	if average_ms >= tick_budget_ms or p95_ms >= tick_budget_ms:
		push_error("STRESS FAILED: simulation work reached the configured %.3f ms tick budget" % tick_budget_ms)
		quit(1)
		return
	quit(0)
